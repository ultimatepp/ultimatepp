...
[{_} 
[ {{10000@(113.42.0) [s0;%% [*@7;4 DropTree]]}}&]
[s1;@(0.0.255)3 &]
...
&]
[s2;%% This class is similar to DropList (widget traditionally named 
`"ComboBox`"), but instead of single list of elements it provides 
...
[s2;%% Triggers when the tree is about to drop. Can be used to populate 
the tree just before it is displayed.&]
[s3; &]
[s4; &]
[s5;:Upp`:`:DropTree`:`:Clear`(`): [@(0.0.255) void]_[* Clear]()&]
[s2;%% Clears the tree and assigns Null as value.&]
[s3; &]
[s4; &]
[s5;:Upp`:`:DropTree`:`:TreeObject`(`): [_^Upp`:`:PopUpTree^ PopUpTree][@(0.0.255) `&]_[* T
reeObject]()&]
[s2;%% Returns TreeCtrl (PopUpTree is implementation detail class 
...
[s2;%% Sets distinct display for selected value. Default is to use 
TreeCtrl`'s Display.&]
[s3;%% &]
[s4; &]
[s5;:Upp`:`:DropTree`:`:DropFocus`(bool`): [_^Upp`:`:DropTree^ DropTree][@(0.0.255) `&]_[* D
ropFocus]([@(0.0.255) bool]_[*@3 b]_`=_[@(0.0.255) true])&]
[s2;%% If active, pressing drop button sets focus to DropTree widget. 
...
[s2;%% Sets MaxHeight with argument computed to display maximum of 
[%-*@3 d] lines with standard font (e.g. text with standard display).&]
[s3;%% &]
[s4; &]
[s5;:Upp`:`:DropTree`:`:DropWidth`(int`): [_^Upp`:`:DropTree^ DropTree][@(0.0.255) `&]_[* D
ropWidth]([@(0.0.255) int]_[*@3 w])&]
[s2;%% Changes the width of popup TreeCtrl to [%-*@3 w] pixels (normally 
it is equal to the width of DropTree widget).&]
[s3;%% &]
[s4; &]
