...
{
	Ptr<O> ptr = object;
	return Cb_<A_>([=](A_a_) { if(ptr) return (ptr->*method)(ap_); });
}
...
{
	Ptr<O> ptr = object;
	return Cb_<A_>([=](A_a_) { if(ptr) return (object->*method)(ap_); });
}
...
{
	return Cb_<A_>([=](A_a_) { return cb(ap_); });
}
#endif
