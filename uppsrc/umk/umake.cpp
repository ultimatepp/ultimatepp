...
CONSOLE_APP_MAIN
{
	...
	if(param.GetCount() >= 2) {
		...
		Puts("umk (U++MaKe) " + version + "\n\n"
		     "Usage:\n"
		     "    umk assembly package [build_method] [--hub-dir dir] [--hub-only] [-options] [+flags] [output]\n\n"
		     "Arguments:\n"
		     "    assembly     - is a direct set of package nest directories relative to working directory that represent U++ assembly separated by ','.\n"
		     "    package      - is the main package (a program to build).\n"
		     "    build_method - is build method that is to be used to build the resulting executable. If not specified, CLANG build method is assumed.\n"
		     "                   Note that in POSIX, umk automatically creates CLANG and GCC build methods if they do not exist.\n"
		     "    Additional optional parameters:\n"
		     "        --hub-dir  - specifies the directory where UppHub packages should be downloaded, using the second parameter, dir, to set the path.\n"
		     "        --hub-only - instructs UMK to handle only the logic related to UppHub.\n"
		     "    Options [-options for example -brU]:\n"
		     "        a - rebuild all.\n"
		     "        b - use BLITZ.\n"
		     "        l - silent mode.\n"
		     "        u - silent mode.\n"
		     ...
		     "        h - enables downloading missing packages from UppHub. This command removes any other UppHub packages if exists.\n"
