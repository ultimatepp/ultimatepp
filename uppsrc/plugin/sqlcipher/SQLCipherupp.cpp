...
class SQLCipherConnection : public SqlConnection, public Link<> {
protected:
	...
	typedef SQLCipherSession::sqlite3 sqlite3;
	typedef SQLCipherSession::sqlite3_stmt sqlite3_stmt;

private:
	SQLCipherSession&  session;
	sqlite3*         db;
	Vector<Value>    param;
	...
	friend class SQLCipherSession;
	void             BindParam(int i, const Value& r);
	void             Reset();

public:
	SQLCipherConnection(SQLCipherSession& the_session, sqlite3 *the_db);
	virtual ~SQLCipherConnection();
};

SQLCipherConnection::SQLCipherConnection(SQLCipherSession& the_session, sqlite3 *the_db)
...
void SQLCipherConnection::Cancel()
{
	if (current_stmt) {
...
void SQLCipherConnection::Reset()
{
	if(current_stmt && sqlite3_reset(current_stmt) != SQLITE_OK)
		session.SetError(sqlite3_errmsg(db), "Resetting statement: " + current_stmt_string, sqlite3_errcode(db));
}

void SQLCipherConnection::SetParam(int i, const Value& r)
{
	LLOG(Format("SetParam(%d,%s)",i,r.ToString()));
	param.At(i) = r;
}

void SQLCipherConnection::BindParam(int i, const Value& r) {
	if (IsNull(r))
...
static int SQLCipherParseForArgs(const char* sqlcmd)
{
   int numargs = 0;
...
bool SQLCipherConnection::Execute() {
	Cancel();
...
int SQLCipherConnection::GetRowsProcessed() const
{
	LLOG("GetRowsProcessed()");
	return sqlite3_changes(db);
}

Value SQLCipherConnection::GetInsertedId() const
{
	LLOG("GetInsertedId()");
	return sqlite3_last_insert_rowid(db);
}

bool SQLCipherConnection::Fetch() {
	ASSERT(NULL != current_stmt);
	if (!got_row_data)
		return false;
	if (got_first_row) {
...
void SQLCipherConnection::GetColumn(int i, Ref f) const {
	ASSERT(NULL != current_stmt);
	if(i == -1) {
...
SqlSession& SQLCipherConnection::GetSession() const { return session; }
String SQLCipherConnection::ToString() const {
	return statement;
}

int SQLCipherSession::ChangePassword(const String& password) {
	int retcode = sqlite3_rekey(db, password, password.GetCount());
...
int SQLCipherSession::CheckDBAccess() {
...
bool SQLCipherSession::Open(const char *filename, const String& password)
{
	ASSERT(NULL == db);
...
SqlConnection *SQLCipherSession::CreateConnection() {
	return new SQLCipherConnection(*this, db);
}

int SQLCipherSession::SqlExecRetry(const char *sql)
{
	ASSERT(NULL != sql);
	ASSERT(0 != *sql);
	int retcode;
...
void SQLCipherSession::Reset()
{
	for(Link<> *s = clink.GetNext(); s != &clink; s = s->GetNext())
		((SQLCipherConnection *)s)->Reset();
}

void SQLCipherSession::Cancel()
{
	for(Link<> *s = clink.GetNext(); s != &clink; s = s->GetNext())
		((SQLCipherConnection *)s)->Cancel();
}

SQLCipherSession::SQLCipherSession()
{
	db = NULL;
	Dialect(SQLITE3);
	busy_timeout = 0;
...
void SQLCipherSession::Rollback() {
	Cancel();
...
Vector<String> SQLCipherSession::EnumDatabases() {
	Vector<String> out;
	Sql sql(*this);
...
Vector<String> SQLCipherSession::EnumTables(String database) {
	Vector<String> out;
	String dbn=database;
...
Vector<String> SQLCipherSession::EnumViews(String database) {
	Vector<String> out;
	String dbn=database;
...
int SQLCipherSession::GetTransactionLevel() const
{
...
Vector<SqlColumnInfo> SQLCipherSession::EnumColumns(String database, String table) {
...
const char *SQLCipherReadString(const char *s, String& stmt) {
	stmt.Cat(*s);
	int c = *s++;
	for(;;) {
		if(*s == '\0') break;
...
bool SQLCipherPerformScript(const String& txt, StatementExecutor& se, Gate<int, int> progress_canceled) {
	const char *text = txt;
	for(;;) {
		String stmt;
		...
		for(;;) {
			...
			if(*text == '\'')
				text = SQLCipherReadString(text, stmt);
			else
			if(*text == '\"')
				text = SQLCipherReadString(text, stmt);
			else
