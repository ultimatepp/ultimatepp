...
Gdb::Gdb()
{
	...
	locals.WhenBar = [this, Mem](Bar& bar) { Mem(bar, locals); };
	watches.NoHeader();
	...
	watches.WhenBar = [this, Mem](Bar& bar) { Mem(bar, watches); WatchMenu(bar); };
	autos.NoHeader();
	autos.AddColumn("", 1);
	autos.AddColumn("", 6);
	autos.EvenRowColor();
	autos.WhenSel = THISBACK1(SetTree, &autos);
	autos.WhenBar = [this, Mem](Bar& bar) { Mem(bar, autos); };
	self.NoHeader();
	self.AddColumn("", 1);
	self.AddColumn("", 6);
	self.EvenRowColor();
	self.WhenSel = THISBACK1(SetTree, &self);
	self.WhenBar = [this, Mem](Bar& bar) { Mem(bar, self); };
	cpu.Columns(3);
