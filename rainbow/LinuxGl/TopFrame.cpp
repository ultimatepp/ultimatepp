...
{
	close.SetImage(LinuxGlImg::close());
	close.EdgeStyle();
	Add(close);
	maximize.SetImage(LinuxGlImg::maximize());
	maximize.EdgeStyle();
	Add(maximize);
...
void TopWindowFrame::Maximize()
{
	if(!maximized && maximize.IsShown()) {
		...
		overlapped = GetRect();
		SetRect(screenRect);
		maximize.SetImage(LinuxGlImg::overlap());
	}
}
...
{
	return maximized ? Rect(0, 0, 0, 0) : ChMargins(LinuxGlImg::border());
}
...
void TopWindowFrame::Paint(Draw& w)
{
	...
	ChPaintEdge(w, sz, LinuxGlImg::border());
	Image img = window->IsForeground() ? LinuxGlImg::title() : LinuxGlImg::bgtitle();
...
{
	Size sz = GetSize();
	Rect m = ChMargins(LinuxGlImg::border());
	Point dir;
