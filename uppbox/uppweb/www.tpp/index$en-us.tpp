...
[ {{6760:3240f0;g0;^l/0r/0t/0b/0 [s2;%% [* U`+`+] is a C`+`+ cross`-platform rapid application development 
framework focused on programmers productivity. It includes a 
set of libraries (GUI, SQL, etc.), and an integrated development 
environment.&]
[s2;%% Rapid development is achieved by the [^topic`:`/`/uppweb`/www`/overview`$en`-us^ s
mart and aggressive use of C`+`+] rather than through fancy code 
generators. In this respect, U`+`+ competes with popular scripting 
languages while preserving C/C`+`+ runtime characteristics.&]
[s2;%% The U`+`+ integrated development environment, TheIDE, introduces 
modular concepts to C`+`+ programming. It features BLITZ`-build 
technology to speedup C`+`+ rebuilds up to 4 times, Visual designers 
for U`+`+ libraries, a [^topic`:`/`/ide`/app`/Topic`$en`-us^ Topic`+`+] 
system for documenting code and creating rich text resources 
...
::-1 [s0; ]
:: [s0; ]
::l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/CtrlLib`/srcdoc`/Tutorial`$en`-us^ GUI 
Tutorial]&]
...
topics]&]
[s4;l192;i150;O0;%% [^topic`:`/`/uppweb`/gsoc`/ideas2014`$en`-us^ GSoC 
ideas]]
::=l0/25r0/25t0/15b0/15R0-1 [s0; ]
:: [s0; ]
::^l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/bazaar`$en`-us^ Bazaar]&]
...
[s4;i150;O0;%% [^https`:`/`/sourceforge`.net`/projects`/upp`/files`/upp`/2015`.2`/^ Cur
...
::l8/25@2-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/gsoc`/gsoc2014`$en`-us^ Google 
Summer of Code resources]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/LearnCpp`$en`-us^ Learn C/C`+`+ 
with TheIDE]]
::l0/25r0/25t0/15b0/15R0 [s0; ]
:: [s0; ]
::^l8/25r8/0t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/extlinks`$en`-us^ External 
links]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]
:: [s0; ]
::^l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/Funding`$en`-us^ Funding U`+`+]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/cookie`$en`-us^ Cookie prize]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]}}&]
[s0; &]
[s0;> [@1;1 Page updated at ][IUPDATETIME;@1;1 UPDATETIME][@1;1  UTC.]]]