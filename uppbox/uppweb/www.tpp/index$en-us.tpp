topic "Ultimate++ is a C++ cross-platform rapid application development framework";
[ $$0,0#00000000000000000000000000000000:Default]
[a13;*R6 $$1,0#31310162474203024125188417583966:caption]
[b42;a42;ph2 $$2,2#45413000475342174754091244180557:text]
[b83;*4 $$3,2#07864147445237544204411237157677:title]
[a25;ph2 $$4,4#49211DBAE19D880B74F16648D25AF532:resource]
[{_} 
[s1;%% Ultimate`+`+ is a C`+`+ cross`-platform rapid application 
development framework&]
[s0;>*R^topic`:`/`/uppweb`/www`/index`$ru`-ru^1%% &]
[ {{6760:3240f0;g0;^l/0r/0t/0b/0 [s2;r58;%% [* U`+`+] is a C`+`+ cross`-platform rapid application development 
framework focused on programmers productivity. It includes a 
set of libraries (GUI, SQL, etc.), and an integrated development 
environment.&]
[s2;r58;%% Rapid development is achieved by the [^topic`:`/`/uppweb`/www`/overview`$en`-us^ s
mart and aggressive use of C`+`+] rather than through fancy code 
generators. In this respect, U`+`+ competes with popular scripting 
languages while preserving C/C`+`+ runtime characteristics.&]
[s2;r58;%% The U`+`+ integrated development environment, TheIDE, 
introduces modular concepts to C`+`+ programming. It features 
BLITZ`-build technology to speedup C`+`+ rebuilds up to 4 times, 
Visual designers for U`+`+ libraries, a [^topic`:`/`/ide`/app`/Topic`$en`-us^ Topic
`+`+] system for documenting code and creating rich text resources 
for applications (like help and code documentation) and [^topic`:`/`/ide`/app`/Assist`$en`-us^ A
ssist`+`+] `- a powerful C`+`+ code analyzer that provides features 
like code completion, navigation and transformation.&]
[s2;r58;%% TheIDE can work with GCC, MinGW and Visual C`+`+ as contained 
in free [^http`:`/`/www`.microsoft`.com`/downloads`/details`.aspx`?familyid`=ff6467e6`-5bba`-4bf5`-b562`-9199be864d29`&displaylang`=en^ W
indows Vista SDK] and contains a full featured debugger. TheIDE 
can also be used to develop non`-U`+`+ applications.&]
[s2;r58;%% [* What you can get with the Ultimate`+`+ download in plain 
English?]&]
[s2;l160;r58;i150;O0;%% Very effective C`+`+ library for cross`-platform 
development in source form.&]
[s2;l160;r58;i150;O0;%% A good integrated development environment, 
designed for developing large C`+`+ applications.&]
[s2;r58;%% You can use both, or you can use whichever you need.]
:: [s1;>/@(0.0.255)0%% &]
[s2;%% &]
[s0;=l64;%% 
@@image:790&890
()
&]
[s2;l64;%% &]
[s2;l64;%% [3 Current release: ][*3 2015.2 (rev ][*IRELEASET;3 release][*3 )]&]
[s2;l64;%% [3 License: ][*3 BSD]&]
[s0;*%% ]}}&]
[s0; &]
[ {{10000f0;g0;l8/25r8/25t8/25b8/25@(240)R(165) [s0;=%% [* U`+`+ Resources]]}}&]
[s0;0 &]
[ {{1229:1995:161:1612:1612:161:1309:1921f0;g0;l8/25t8/25b8/25@(240)R(165) [s0; [*2 Getting Started]]
::l0/25r8/25 [s0;>%% [@(114)2 5 minutes to learn the basics...]&]
[s0;>%% [@(114)2 ... and more]]
::r0/25t0/25b0/25@2 [s0;>@(114)2%% ]
::l8/25t8/25b8/25@(240) [s0;%% [*2 Users]]
::l0/25r8/25-3 [s0;>@(114)2%% ]
::r0/25t0/15b0/15@2R0-2 [s0; ]
::-1 [s0; ]
:: [s0; ]
::l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/CtrlLib`/srcdoc`/Tutorial`$en`-us^ GUI 
Tutorial]&]
[s4;i150;O0;%% [^topic`:`/`/ide`/app`/GettingStarted`$en`-us^ TheIDE 
basics]&]
[s4;i150;O0;%% [^topic`:`/`/ide`/app`/GettingStarted`$en`-us^ Skylark 
Tutorial (Web framework)]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/comparison`$en`-us^ Comparisons]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/overview`$en`-us^ Overview]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/tutoring`$en`-us^ Tutoring 
Plan]]
::l0/25r0/25t0/15b0/15R0 [s0; ]
::r8/25t/25b/25R(165) [s0;>@(114)2%% ]
::^t8/25b8/25-2 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/documentation`$en`-us^ Documentation]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/examples`$en`-us^ Examples]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/videotutorials`$en`-us^ Video 
Tutorials]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/SupportedPlatforms`$en`-us^ Supported 
platforms]]
::=r0/25t0/15b0/15R0-1 [s0; ]
:: [s0; ]
::^l8/25r8/0t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/apps`$en`-us^ Applications]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/FAQ`$en`-us^ FAQ]&]
[s4;i150;O0;%% [^http`:`/`/www`.ultimatepp`.org`/forums^ Forums]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]
::t8/25b8/25R(165)-1 [s0;*+34 ]
::t0/15b0/15R0 [s0; ]
::t/25b/25R(165) [s0;>@(114)+34%% ]
::t8/25b8/25-1 [s0;*+34 ]
::t0/15b0/15R0 [s0; ]
::t8/25b/25R(165)|5 [s0;*+34 ]
::b8/25-1 [s0;*+34 ]
::t0/15b0/15R0 [s0; ]
::l8/25t8/25b8/25@(240)R(165) [s0;%% [*2 Developers]]
::l0/25r8/25-3 [s0;>%% [@(114)2 If you feel like contributing to web or code this 
is your place]]
::r0/25t0/15b0/15@2R0-2 [s0; ]
::-1 [s0; ]
:: [s0; ]
:: [s0; ]
::l8/25r/0t8/25b8/25@(240)R(165) [s0;%% [*2 Download U`+`+]]
::l0/25r8/25 [s0;>%% [@(114)2 Let`'s go!]]
::^l8/25@2-2 [s4;i150;O0;%% [^http`:`/`/www`.ultimatepp`.org`/redmine`/projects`/upp^ Project 
management (Redmine)]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/svnInstall`$en`-us^ SVN install 
and permissions]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/bugreports`$en`-us^ Bug report. 
Submitting a patch]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/contribweb`$en`-us^ Web page]&]
[s4;i150;O0;%% New development ideas&]
[s4;l192;i150;O0;%% [^topic`:`/`/uppweb`/www`/currenttopics`$en`-us^ Current 
topics]&]
[s4;l192;i150;O0;%% [^topic`:`/`/uppweb`/gsoc`/ideas2014`$en`-us^ GSoC 
ideas]]
::=l0/25r0/25t0/15b0/15R0-1 [s0; ]
:: [s0; ]
::^l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/bazaar`$en`-us^ Bazaar]&]
[s4;i150;O0;%% [^topic`:`/`/ide`/app`/About`$en`-us^ Authors `& License]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]
:: [s0; ]
::^l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/download`$en`-us^ Download]&]
[s4;i150;O0;%% [^https`:`/`/sourceforge`.net`/projects`/upp`/files`/upp`/2015`.2`/^ Cur
rent release]: [IRELEASE; release]&]
[s4;i150;O0;%% [^http`:`/`/www`.ultimatepp`.org`/www`$uppweb`$nightly`$en`-us`.html^ La
test svn]: [ILATESTSVN; latestsvn]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]
::r/0t8/25b8/25R(165)-1 [s0;*+34%% ]
::r/25t0/15b0/15R0 [s0; ]
::r/0t8/25b/25R(165)|2 [s0;*+34%% ]
::b8/25-1 [s0;*+34%% ]
::r/25t0/15b0/15R0 [s0; ]
:: [s0; ]
::r/0t8/25b8/25R(165)-1 [s0;*+34%% ]
::r/25t0/15b0/15R0 [s0; ]
::l8/25r/0t8/25b8/25@(240)R(165) [s0;%% [*2 Students]]
::l0/25r8/0 [s0;=@(114)2%% ]
::r0/25t0/15b0/15@2R0 [s0; ]
::l8/25r/0t8/0b8/0@(240)R(165) [s0;%% [*2 Other]]
::l0/0r8/0 [s0;=*2%% ]
::l/25r0/25t0/15b0/15@2R0 [s0; ]
::l8/25t8/25b8/25@(240)R(165) [s0;%% [*2 Supporting U`+`+]]
::l0/25r8/25 [s0;>%% [@(114)2 Sometimes more than code is needed...]]
::l8/25@2-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/gsoc`/gsoc2014`$en`-us^ Google 
Summer of Code resources]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/LearnCpp`$en`-us^ Learn C/C`+`+ 
with TheIDE]]
::l0/25r0/25t0/15b0/15R0 [s0; ]
:: [s0; ]
::^l8/25r8/0t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/extlinks`$en`-us^ External 
links]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]
:: [s0; ]
::^l8/25r8/25t8/25b8/25R(165)-1 [s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/Funding`$en`-us^ Funding U`+`+]&]
[s4;i150;O0;%% [^topic`:`/`/uppweb`/www`/cookie`$en`-us^ Cookie prize]]
::=l0/25r0/25t0/15b0/15R0 [s0; ]}}&]
[s0; &]
[s0;> [@1;1 Page updated at ][IUPDATETIME;@1;1 UPDATETIME][@1;1  UTC.]]]