...
I admit, that to `'learn`' U`+`+ style isn`'t done in a day because 
it isn`'t a fixed law. One needs to have worked with it for a 
while and feel some comfort with it. But this document should 
shorten the time.&]
[s0; &]
...
[s0; [*+117 Design]&]
[s0;+117 &]
[s0;i150;O0; Produce clean, `'intuitive`' and compact code, making 
...
[s0; &]
[s0;i150;O0; [* Include relative paths] instead of absolute in [C #include]s, 
...
Example:&]
[s0; &]
[s0;i160; [C void Enable(bool e `= true) `{ /`*your code`*/ `}]&]
[s0;i160; [C void Disable() `{ return Enable(false); `}]&]
[s0;i160; [C bool IsEnabled() const `{ /`*your code`*/ `}]&]
[s0;i160;C &]
[s0;i160; Upp code is both easier to read and to write because of 
...
[s0; [*+117 Readability]&]
[s0; &]
[s0;i150;O0; The [* default code style in TheIDE] is the first reference 
(Edit > Advanced > Format code in editor).&]
[s0; &]
[s0;i150;O0; [* Use Pascal Case] for function names. (concatenated 
...
example)&]
[s0; &]
[s0;i150;O0; Put spaces around operators, example: [C if(a > 12) `{ 
/`*code`*/ `}]&]
[s0; &]
[s0;i150;O0; The ampersand belongs to the object. example: [C Ctrl`& 
GetCtrlRef();]&]
[s0; &]
