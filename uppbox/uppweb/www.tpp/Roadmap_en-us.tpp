...
[s0;i150;O0;%% Moveable concept redesigned with C`+`+17 features. 
U`+`+ now allows non`-moveable types to be stored in Vector flavor 
...
[s0;i150;O0;%% Some iffy code now made more C`+`+ compliant (e.g. 
always using memcpy for unaligned data)&]
...
[s5;i150;poO0;%% gtk backend improvements, XWayland mouse cursor 
bug workaround&]
[s5;po%% &]
[s5;po%% [* CtrlLib]&]
[s5;po%% &]
...
to the final image.&]
[s5;i150;poO0;%% Output directory in assembly definition now can 
be left empty and defaults to reasonable path.&]
...
[s5;i150;poO0;%% Compare with menu now suggests files in Download 
folder too&]
...
[s0;%% &]
[s0;%% The new release brings some long awaited features. As you 
might remember for the last release we removed the old deprecated 
library for code formatting. In the current release formatting 
is back based on clang`-format. Also TheIDE has got a special 
dialog when you can adjust formatting to your needs. Formatting 
is available under `"Assist`" `-> `"Reformat`" menu.&]
[s0;%% &]
[s0;%% macOS support, temporarily removed in previous release, is 
back. Our POSIX installation is now universal, covering Linux, 
BSDs and MacOS, which means that when you unpack it on macOS 
you just run ./install script to download all dependencies using 
brew and build umk and theide just like on other POSIX platforms. 
...
[s0;%% Core&]
[s0;%% &]
[s0;i150;O0;%% MacOS is supported again; POSIX and macOS releases 
are now merged to single archive&]
...
[s5;%% &]
[s5;%% We would like to thank all of our amazing contributors and 
users! Without your help we wouldn`'t be in the place we are 
today. Also, many thanks to our UppHub contributors who do a 
great job. Your additional packages are very helpful and needed 
for our users.&]
[s0;ph*%% &]
[s0;ph%% [* Release highlights]&]
[s5;%% &]
[s5;l128;i150;O0;%% Homegrown C`+`+ parser in TheIDE is replaced 
with LibClang resulting in new and improved Assist`+`+ features&]
...
[s5;l128;i150;O0;%% Fix issue with not working drag and drop for 
files on Windows&]
[s5;%% &]
[s3;%% 2022.2 (rev. 16270) (May 2022)&]
[s5;po*/%% &]
[s5;po*/%% &]
[s5;%% This release serves mainly as a service pack to 2022.1 and 
contains significant number of resolved issues. There are limited 
set of new features.&]
[s0;ph%% [* Release highlights]&]
...
[s5;l128;i150;O0;%% CtrlLib: Fixed issues with namespace agnostic 
layouts&]
...
[s5;l288;%% To support host APIs, char16 is introduced (but there 
is no 16`-bit String varian).&]
...
[s5;l128;i150;O0;%% Improved input method (aka preedit in Linux, 
aka marked text in MacOS) support&]
[s5;l128;i150;O0;%% ConvertFloat and EditFloat to handle float vs 
...
[s5;*%% &]
[s5;%% [* POSIX]&]
[s5;l128;i150;O0;%% In order to satisfy UppHub requirements, Git 
...
[s5;po%% &]
[s5;po%% [* Core]&]
[s0;l160;i150;O0;%% double NAN and INF values are now considered 
Null (IsNull returns true for them).&]
...
[s0;l160;i150;O0;%% U`+`+ does not require MemoryFreeThread call 
at the end of thread (it is now implemented using thread`_local 
destructor)&]
...
[s0;l160;i150;O0;%% JPEG raw image data is now exported to PDF as 
JPEG (instead of recoding)&]
[s0;%% &]
[s0;%% [* TheIDE]&]
...
[s0;l160;i150;O0;%% Repo history of .upp files directly available 
via package context menu&]
...
[s0;l160;i150;O0;%% Some common nest root files like README.md or 
LICENSE are now directly accessible through <meta> pseudo`-package.&]
...
[s5;%% &]
[s5;po%% [* Release highlights]&]
[s5;l160;i150;poO0;%% This release is mostly about optimizations, 
...
[s5;%% &]
[s0;ph%% [* ScatterDraw]&]
[s5;l160;i150;poO0;%% Added SetDataSourceInternal() to internally 
host data&]
[s5;%% &]
[s5;%% [* ScatterCtrl]&]
...
[s0;l160;i150;O0;%% plugin/Eigen to master branch commit C1D944DD 
(9/May/2020)&]
[s5;%% &]
...
[s5;l160;i150;poO0;%% In Win32, PDB debugger now displays various 
U`+`+ and standard library containers and types nicely&]
[s5;l160;i150;poO0;%% Win32 release now ships with CLANG compiler 
toolchain&]
...
[s5;l160;i150;poO0;%% SQL`_RENAME .sch `'command`' can be used to 
have different name of column in DB and code.&]
[s5;po%% &]
[s5;po%% [* uppsrc]&]
[s5;l160;i150;poO0;%% In POSIX, external library dependencies are 
now refactored with pkg`-config&]
[s24;%% &]
...
[s5;l160;i150;poO0;%% Fast U`+`+ allocator is now used internally 
where possible even if standard allocator us used for new/delete 
(with USEMALLOC flag)&]
...
[s5;l160;i150;poO0;%% GLCtrl new method ExecuteGL to execute code 
with correct GL context&]
...
[s5;l160;i150;poO0;~~~192;%% ToAscii, ToUpperAscii, ToLowerAscii 
functions optimized&]
...
[s5;l160;i150;poO0;~~~192;%% mingw64 now using lld linker `- link 
times with mingw are now practically zero&]
...
images into PDF&]
[s22;%% GUI programming&]
[s5;l160;i150;poO0;%% In Windows, owned windows now have minimize 
box disabled (as minimizing of owned windows leads to strange 
behaviour)&]
[s5;l160;i150;poO0;%% LineEdit`::WhenScroll event&]
[s5;l160;i150;poO0;%% New OptionBox widget (Option with LabelBox 
like graphics, also able to automatically enable/disable contained 
widgets)&]
[s5;l160;i150;poO0;%% FileSel now has option to add widgets into 
the dialog (e.g. to specify save format)&]
[s22;%% TheIDE&]
[s5;l160;i150;poO0;%% Very long files (>400MB) are now opened for 
...
[s5;l160;i150;poO0;%% Bits class optimized, it is now possible to 
set multiple bits (count parameter)&]
...
[s5;l160;i150;poO0;%% Server Name Indicator support in Socket and 
HttpRequest&]
...
style is not normal&]
[s22;%% TheIDE&]
[s5;l192;i150;poO0;%% Improved debugging of console applications 
with MinGW&]
...
on that platform.&]
[s22;%% Core&]
[s5;l160;i150;poO0;%% Original U`+`+ Callbacks are deprecated and 
replaced with Function with better lambda support.&]
...
[s5;l160;i150;poO0;%% CoWorkerResources class now provides `'per 
worker thread`' context.&]
[s5;l160;i150;poO0;%% CoPartition algorithm useful for partitioning 
array for parallel programming.&]
[s5;l160;i150;poO0;%% Where it makes sense, U`+`+ algorithms now 
have parallel variant.&]
...
document.&]
[s5;l160;i150;poO0;%% ArrayCtrl got SetLineColor and GetCellRect 
methods.&]
[s22;%% TheIDE&]
[s5;l160;i150;poO0;%% Comments now can be spell checked and there 
is comment word wrap and reformatting operation.&]
...
lines are now shown.&]
[s5;l160;i150;poO0;%% Most U`+`+ designers now support drag`&drop 
to reorganize lists.&]
[s5;l160;i150;poO0;%% Find File window was remastered.&]
[s5;l160;i150;poO0;%% New dialog for creating package file.&]
[s5;l160;i150;poO0;%% Android application can be directly launch 
on emulator from TheIDE.&]
...
[s5;l160;i150;poO0;%% New QTF command [*@5 `{`{`*] is shortcut for 
[*@5 `{`{`~0/0] to simplify using invisible tables for organizing 
text.&]
[s5;l160;i150;poO0;%% PdfDraw now supports urls (e.g. when converting 
QTF/RichText to PDF).&]
[s5;l160;i150;poO0;%% RichText/QTF now support round borders for 
table cells.&]
...
[s5;l160;i150;poO0;%% Editor now shows [* misplaced whitespaces] in 
source files.&]
[s5;l160;i150;poO0;%% TheIDE now detects binary files and provides 
binary viewer.&]
[s5;l160;i150;poO0;%% Toolbar has new navigation icons and icons 
to switch editation mode (text/designer/binary).&]
...
[s22;%% Win32 releases&]
[s5;l160;i150;poO0;%% Win32 now does not come as .exe installer, 
but simple .7z archive, which acts as `"[* portable]`" installation. 
...
[s22;%% Core&]
[s5;l160;i150;poO0;%% U`+`+ abandoned `"default pick`" semantics 
for containers. The transfer semantics now has to be specified 
explicitly, either as `'[* pick]`' or `'[* clone]`' (this is enforced 
in C`+`+11 mode, in legacy mode old behaviour is maintained and 
pick/clone are optional).&]
...
[s5;l160;i150;poO0;%% Initial round of [* C`+`+11] support (lambda 
callbacks, minor fixes)&]
[s5;l160;i150;poO0;%% A set of small utility functions [* findarg], 
[* decode], [* min]/[* max] now supporting more arguments. [* SplitTo 
]and [* Merge] for splitting/merging strings.&]
...
speed optimized.&]
[s5;l160;i150;poO0;%% Value invalid casts now throwing exception 
ValueTypeError instead of panic.&]
...
adapted as package&]
[s22;%% GUI programming `& graphics&]
[s5;l160;i150;poO0;%% [* GTK backend] (U`+`+ now using Gtk for host 
platform interface)&]
[s5;l160;i150;poO0;%% [* EditDateDlg], [* EditNumber] simple function 
for entering single value (equivalent to EditText)&]
...
[s5;l160;i150;poO0;%% LineEdit now able to work with [* rectangular 
selections.]&]
...
[s5;l160;i150;poO0;%% AttrText is now rich comparable Value: it is 
now possible to sort ArrayCtrl by AttrText filled columns.&]
...
[s5;l160;i150;poO0;%% [* RichText/RichEdit] header/footer support, 
QTF now using BASE64 for binary data (instead of 7`-bit raw encoding).&]
[s5;l160;i150;poO0;%% [* Turtle ]is another web framework of U`+`+ 
`- this one allows to convert existing GUI applications into 
Web application: simple Java Script terminal connects to server 
where GUI application runs.&]
...
[s5;l160;i150;poO0;%% SqlSetFrom functions creates a SqlSet from 
any container.&]
...
[s5;l160;i150;poO0;%% Find/replace GUI refactored, added support 
for incremental finds, added regexp support.&]
[s5;l160;i150;poO0;%% Block replace now has grep`-like feature to 
remove matching/non`-matching lines (instead of replacing matches)&]
...
`"Array`").&]
[s5;l160;i150;poO0;%% Compilation errors now parsed and displayed 
nicely.&]
[s5;l160;i150;poO0;%% Assist`+`+ context goto (Alt`+J) now supports 
local variables (goes to declaration).&]
[s5;l160;i150;poO0;%% Layout/Icon designers now have find Search 
filter.&]
...
[s5;l160;i150;poO0;%% Layout designer now can sort widgets based 
on their graphical position.&]
...
[s3;%% 5485[3  (Oct 2012)]&]
[s0;3%% &]
[s0;%% The focus of this release was server programming. The main 
new feature is `"[* Skylark]`", lean and mean [^http`:`/`/www`.ultimatepp`.org`/srcdoc`$Skylark`$Tutorial`$en`-us`.html^ U
...
[s0;i150;O0;%% TCP/IP and HTTP was refactored and integrated into 
Core, providing new TcpSocket, IpAddrInfo, HttpHeader and HttpRequest 
...
[s0;i150;O0;%% .ini parameters now can be optionally encapsulated 
using INI`_`* helpers.&]
...
(new mode).&]
[s0;i150;O0;%% Sql: SqlLoadTable and SqlLoadColumn helper function 
can read whole tables into C`+`+ VectorMaps.&]
...
[s0;%% [* CtrlLib]&]
[s0;%% &]
[s0;i150;O0;%% EditField now has WhenPasteFilter that can be used 
to change pasted clipboard text before inserting it.&]
...
[s0;%% GUI for trivial RGBA framebuffer (with keyboard and mouse 
inputs).&]
[s0;%% &]
[s0;%% New command line utility `"umk`" `- GUI independent, U`+`+ 
package builder: umk.&]
[s0;%% &]
...
[s0;i150;O0;%% Layout designer now has `'struct`' option in code 
generator&]
...
[s0;i150;O0;%% Value now has operator`[`] (key) and operator`[`] 
(index) methods to directly access contained ValueMap and ValueArray 
elements.&]
...
[s0;i150;O0;%% ArrayCtrl now has AsText, SetClipboard, AsQtf and 
AsCsv methods.&]
...
[s0;i150;O0;%% Main package configuration editor shows structured 
flags accepted by packages.&]
...
operator(). &]
[s0;i150;O0;%% New SqlMassInsert provides generic acceleration of 
mass inserts using `'union all`' trick. &]
[s0;i150;O0;%% IsSame is a new SqlExp equality test which is null 
safe (null `=`= null yields true).&]
...
/ Decrypter.&]
[s0;i150;O0;%% BufferStream: Stream based on Vector<byte> buffer, 
self growing.&]
...
[s0;%% &]
[s0;%% Functions4U&]
[s0;i150;O0;%% Added functions, GatherTpp, improved doc and fixed 
bugs&]
[s0;%% &]
[s0;%% SysInfo&]
...
[s0;i150;O0;%% MtAlt: Added support for fast 5`-argument callbacks 
(copy and pick version).&]
[s0;i150;O0;%% Form: Form core package, Editor and sample.&]
[s0;i150;O0;%% Scatter: Added SetSequentialX and SetFastViewX for 
faster refresh.&]
...
[s0;%% &]
[s0;%% [* TheIDE]&]
[s0;i150;O0;%% Packages now sorted by directory closeness to main 
package directory, names later `- puts main package nest or main 
package subpackages first in the list&]
[s0;%% &]
[s0;%% [* Library]&]
[s0;%% &]
[s0;%% Core:&]
...
[s0;i150;O0;%% SqlCtrl: SqlArray has new WhenFilter Gate (able to 
filter records being fetched)&]
[s0;%% &]
[s0;%% Sql:&]
...
[s0;i150;O0;%% FileSel was improved by adding Places with common 
directories.&]
[s0;%% &]
[s0;%% [* Library]&]
[s0;*%% &]
...
[s0;i150;O0;%% XmlParser has got new Peek methods. Socket has new 
GetPeerAddr method.&]
[s0;i150;O0;%% CParser now provides the information about current 
column.&]
[s0;%% &]
[s0;%% CtrlLib:&]
...
reworked&]
[s0;i150;O0;%% Current editor line is now highlighted in the left 
bar.&]
[s0;%% &]
[s0;%% [* Library]&]
[s0;*%% &]
...
[s0;%% &]
[s0;%% Painter:&]
[s0;i150;O0;%% Image rescaling algorithm was improved for better 
downscaling. &]
[s0;%% &]
[s0;%% Core: &]
...
[s0;i150;O0;%% Functions4U: New package with additional functions 
to complement Core and CtrlLib.&]
...
[s0;i150;O0;%% SliderCtrl now allows reversed orientation (thanks 
avpavp!)&]
[s0;%% &]
[s0;%% RichEdit:&]
[s0;i150;O0;%% New much more effective speller dictionary format, 
utilities required to convert OpenOffice speller dictionaries 
into U`+`+ speller files.&]
[s0;i150;O0;%% Labeled paragraphs now marked with arrow displayed 
in the left bar.&]
...
[s0;%% &]
[s0;%% Core:&]
[s0;i150;O0;%% In Win32, console applications no longer pop up a 
MessageBox when panicking, instead they write the panic string 
on the error output and terminate.&]
[s0;%% &]
[s0;%% GridCtrl:&]
[s0;i150;O0;%% Multiline popups now allowed, using GridDisplay to 
paint the content.&]
[s0;%% &]
[s0;%% Bazaar:&]
...
[s0;i150;O0;%% PostgreSQL: GetInsertedId now supported for tables 
with primary key `'id`'&]
[s0;i150;O0;%% CtrlLib: MenuBar WhenOpenSubMenu, WhenCloseSubMenu 
callbacks&]
[s0;i150;O0;%% Sql, SqlExp: Sql`::InsertNoKey(Fields) ommits first 
column `- intended for key with autoincrement&]
[s0;%% &]
[s0;%% [* TheIDE]&]
[s0;*%% &]
...
[s0;%% &]
[s0;i150;O0;%% Draw&]
[s0;l160;i160;O3;%% DisplayWithIcon (Display that adds an icon to 
any other Display)&]
...
[s0;i150;O0;%% Geom: Added new Computational Geometry algorithm: 2D 
convex hull calculation (ConvexHullOrder)&]
...
[s0;%% &]
[s0;%% Major overhaul graphics infrastructure is finished, U`+`+ 
is now capable of headless drawing operations&]
[s0;%% (e.g. drawing graphics and text into .png files in console 
applications, without X11)&]
[s0;%% &]
[s0;%% Minor improvements:&]
[s0;%% &]
[s0;%% [* Library]&]
[s0;*%% &]
[s0;i150;O0;%% RegExp fixed to work with mingw, added GetMatchPos 
method&]
...
[s0;i150;O0;%% Non`-fixed pitch fonts allowed in editor (used as 
fixed...)&]
[s0;4%% &]
...
[s0; [* Library]&]
[s0;* &]
[s0;i150;O0;%% Change in SetDateFilter format, cleanup in related 
docs &]
[s0;i150;O0;%% Improved translation docs &]
[s0;i150;O0;%% Added runtime check for serializing RichValue with 
missing Register &]
...
[s0; [* TheIDE ]&]
[s0;* &]
[s0;i150;O0;%% Refactored CodeNavigator, search symbol (Ctrl`+Q) 
now copies current id search field &]
[s0;*4%% &]
...
[s0;%% [* TheIDE]&]
[s0;%% &]
[s0;i150;O0;%% Topic`+`+ documentation editor now integrated into 
main window&]
...
[s0;%% [* TheIDE]&]
[s0;%% &]
[s0;i150;O0;%% New Icon designer (supports alpha channel and has 
tools to produce antialiased icons)&]
[s0;%% &]
[s0;%% [* Distribution]&]
[s0;*%% &]
...
[s0;%% [* Distribution]&]
[s0;*%% &]
[s0;i150;O0;%% new Windows installer `- download size reduced by 
50%&]
[s0;%% &]
[s0;*3%% &]
...
[s0;i150;O0;%% Package organizer redesigned, `"When`" conditions 
now support logical operators, `"uses`" can be conditional&]
...
[s0;i150;O0;%% Sqlite was added as source`-based plugin together 
with U`+`+ adapter&]
...
[s0;%% [* Distribution]&]
[s0;%% &]
[s0;i150;O0;%% U`+`+/Win now ships with SDL library and contains 
nice SDLExample as well as SDL project templates.&]
