...
[s0;#2 &]
[s0;# [2 Our difference with other frameworks is the deep use of C`+`+ 
...
[s0;%- &]
[ {{10000f0;g0;^l/0t/25b4/25 [s0;# [*2 What is the main development mailing list for your organization? 
This question will be shown to students who would like to get 
more information about applying to your organization for GSoC 
2010. If your organization uses more than one list, please make 
sure to include a description of the list so students know which 
to use.]]}}&]
[s0; [2 Our project has a Forum here:]&]
...
[s0;i150;O0; [2 Proposals, including:]&]
...
possible]]}}&]
[s0;# [2 To get experience with students and new programmers in general, 
we launched last year a `"][^topic`:`/`/uppweb`/www`/tutoring`$en`-us^2 Tutoring 
Plan][2 `" (http://www.ultimatepp.org/www`$uppweb`$tutoring`$en`-us.html) 
inspired in GSoC.]&]
[s0;#2 &]
[s0;# [2 This plan gives two weeks help for free giving a specially 
personal support and introducing the person to the Forum.]&]
[s0;#2 &]
[s0;# [2 All the requirements for tutors are in ][^http`:`/`/www`.ultimatepp`.org`/www`$uppweb`$tutoringmanual`$en`-us`.html^2 T
utoring Manual][2  page (http://www.ultimatepp.org/www`$uppweb`$tutoringmanual`$en`-
us.html) and in ][^http`:`/`/www`.ultimatepp`.org`/www`$uppweb`$tutoringtutor`$en`-us`.html^2 T
utoring Page][2  (http://www.ultimatepp.org/www`$uppweb`$tutoringtutor`$en`-us.html)
.]&]
[s0;#2 &]
[s0;# [2 These requirements are very similar to the ones of GSoC, so 
all Tutoring Plan tutors are the main source of GSoC mentors. 
However all people that comply with the later requirements will 
be accepted. ]&]
[s0;#2 &]
...
[s0;#2 &]
[s0;# [2 However if a student does not answer we will try our best 
by email or other means to know the real reason of the failure. 
In that case we would try to get to an agreement with for example 
a reasonably more flexible schedule or goals.]&]
[s0;#2 &]
...
[s0;#2 &]
[s0;# [2 In addition all the mentorship process will be public in Wiki. 
In fact Wiki will include not only project roadmap and weekly 
reports, but daily state so it will be used like a blog.]&]
[s0;#2 &]
[s0;# [2 This will increase the project quality as student and mentor 
will be followed by community. But in the other side the community 
will give them support as all project process is public.]&]
[s0;#2 &]
[s0;# [2 This way the mentor effort could be temporary released as 
community can do his/her job.]&]
[s0;#2 &]
...
members no matter their skills.]&]
[s0;#2 &]
[s0;# [2 All the efforts will be done to let the project results to 
be included into main U`+`+ source base. In fact the mentor job 
will begin weeks before GSoC as s/he will be in selection process, 
...
project.]&]
[s0;#2 &]
[s0;# [2 After nice and accurate answers from Leslie and Carol, we 
cheered up with `"][^http`:`/`/www`.ultimatepp`.org`/www`$uppweb`$cookie`$en`-us`.html^2 C
ookie Prize][2 `" (http://www.ultimatepp.org/www`$uppweb`$cookie`$en`-us.html) 
for the nicest U`+`+ contributor. Later we decided to learn from 
experience by launching the `"][^http`:`/`/www`.ultimatepp`.org`/www`$uppweb`$tutoring`$en`-us`.html^2 T
utoring Plan][2 `" (http://www.ultimatepp.org/www`$uppweb`$tutoring`$en`-us.html) 
to give the kindest support to new U`+`+ users and learning how 
to be good Tutors.]&]
[s0;#2 &]
