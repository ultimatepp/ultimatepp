...
[s2;%% Starts a new process defined by the command [%-*@3 cmd] with 
arguments list [%-*@3 arg], using [%-*@3 envptr] environment variables 
and running the process in [%-*@3 dir] directory.&]
[s2;%% All text output from process to [^https`:`/`/en`.wikipedia`.org`/wiki`/Standard`_streams^ s
tandard streams] stdout and stderr is mixed and can be got using 
Read() method.&]
[s3;%% &]
...
[s2;%% In a command window, this text would be entered using the 
keyboard.&]
[s3;%% &]
[s4; &]
[s5;:Upp`:`:LocalProcess2`:`:Read`(Upp`:`:String`&`): [@(0.0.255) bool]_[* Read]([_^Upp`:`:String^ S
tring][@(0.0.255) `&]_[*@3 res])&]
[s2;%% Reads text [%-*@3 res] from process. This process should have 
to be started using Start() function.&]
...
[s2;%% Reads text [%-*@3 reso] from stdout and [%-*@3 rese] from stderr. 
This process should have to be started using Start2() function.&]
[s2;%% In a command window, this text would be output to the screen.&]
[s3;%% &]
[s4; &]
...
[s2;%% Pauses/freezes/suspends process and all children processes 
and threads.&]
