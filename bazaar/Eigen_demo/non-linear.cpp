...
void NonLinearOptimization() {
	...
	puts("(Given a set of non linear equations and a set of data longer than the equations, "
...
void NonLinearSolving() {
	...
	puts("(Finds a zero of a system of n nonlinear equations in n variables)");
	...
	if (solver.solveNumericalDiff(x) != 1)
		...
		if (VerifyIsApprox(x[0], -0.5706545) && 
			...
			puts("Equation solving is right!");
	}
}
