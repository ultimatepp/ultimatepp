...
//	Puts(Format("Root:             %s", GetRootFolder()));
	...
	if (!GetMemoryInfo(memoryLoad, totalPhys, freePhys, totalPageFile, freePageFile, totalVirtual, freeVirtual))
		...
		Puts(Format("Total physical memory:    %s bytes (%s)", Format64(totalPhys), BytesToKMGT(totalPhys)));
		Puts(Format("Free physical memory:     %s bytes (%s)", Format64(freePhys), BytesToKMGT(freePhys)));	
		Puts(Format("Total paging file:        %s bytes (%s)", Format64(totalPageFile), BytesToKMGT(totalPageFile)));	
		Puts(Format("Free paging file:         %s bytes (%s)", Format64(freePageFile), BytesToKMGT(freePageFile)));	
		Puts(Format("Total virtual memory:     %s bytes (%s)", Format64(totalVirtual), BytesToKMGT(totalVirtual)));	
		Puts(Format("Free virtual memory:      %s bytes (%s)", Format64(freeVirtual), BytesToKMGT(freeVirtual)));
	}
	Puts("\nOs info:");
	...
	for (int i = 0; i < drives.GetCount(); ++i) {
		...
		if (mounted) {
			...
			if(!GetDriveSpace(drives[i], freeBytesUser, totalBytesUser, totalFreeBytes))
				...
				Puts(Format(" Free Bytes User:  %s (%s)", Format64(freeBytesUser), BytesToKMGT(freeBytesUser)));
				Puts(Format(" Total Bytes User: %s (%s), Total Free Bytes: %s (%s)", Format64(totalBytesUser), BytesToKMGT(totalBytesUser), Format64(totalFreeBytes), BytesToKMGT(totalFreeBytes)));
			}
		} else
			Puts (" Not mounted");
	}
	...
	Window_SaveCapture(0, "c:\\Desktop.bmp");
	Puts ("Screenshot saved in c:\\Desktop.bmp");
#endif
	...
	LaunchFile(fileTest);
	{
		...
		if (windowId != -1) {
			...
			Puts(Format("Editor window is located in %d, %d, %d, %d", (int)left, (int)top, (int)right, (int)bottom));
    		...
    		Window_SaveCapture(windowId, "c:\\Windowgrab.bmp");
#endif
		}
	}
