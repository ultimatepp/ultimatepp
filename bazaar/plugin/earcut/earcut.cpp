...
private:
    struct Node {
        Node(int index, double x_, double y_) : i(index), x(x_), y(y_) {}
    ...
    Node* insertNode(int i, const Pointf& p, Node* last);
...
Earcut::eliminateHoles(const Polygon& points, Node* outerNode) {
    ...
    Sort(queue, [](const Node* a, const Node* b) {
        return a->x < b->x;
    });
...
typename Earcut::Node*
Earcut::insertNode(int i, const Pointf& pt, Node* last) {
    Node* p = nodes.construct(i, pt.x, pt.y);
