...
void MultiList::MouseWheel(Point p, int zdelta, dword keyflags) {
	sb.Wheel(zdelta);
}

int  MultiList::GetColumnCx(int i) const {
	if (mode == MODE_ROWS)
		return cx;
	int szcx = GetSize().cx;
...
int MultiList::GetColumnItems() const {
	if(cy == 0) return 0;
	return GetSize().cy / cy;
}

int  MultiList::GetDragColumn(int x) const {
	if (mode == MODE_ROWS)
		return -1;	
	int cx = GetColumnCx();
	int i = 0;
...
Image MultiList::CursorImage(Point p, dword)
{
	return HasCapture() || GetDragColumn(p.x) >= 0 ? Image::SizeHorz() : Image::Arrow();
}

int MultiList::GetItem(Point p)
{
	int i;
...
void MultiList::DoClick(Point p, dword flags)
{
	SetWantFocus();
	PointDown(p);
	p.y %= cy;
...
void MultiList::LeftDown(Point p, dword flags) {
	selclick = false;
...
void MultiList::LeftUp(Point p, dword flags)
{
	if(selclick)
		DoClick(p, flags);
	selclick = false;
...
void MultiList::LeftDrag(Point p, dword keyflags)
{
	if(!HasCapture())
		WhenDrag();
}

void MultiList::RightDown(Point p, dword flags) {
	int i = GetItem(p);
	if(!(i >= 0 && i < GetCount() && multi && IsSel(i)))
		PointDown(p);
...
void MultiList::LeftDouble(Point p, dword flags) {
	LeftDown(p, flags);
	WhenLeftDouble();
}

void MultiList::SyncInfo()
{
...
void MultiList::MouseMove(Point p, dword)
{
	if(HasCapture()) {
...
void MultiList::MouseLeave()
{
	if(!info.IsOpen())
		info.Cancel();
}

Rect MultiList::GetItemRect(int i) const {
	if (mode == MODE_ROWS)
...
dword MultiList::SwapKey(dword key)
{
	if (mode == MODE_ROWS) {
		switch(key) {
		case K_UP:
...
bool MultiList::Key(dword _key, int count) {
	int c = cursor;
...
void MultiList::SelectOne(int i, bool sel) {
	if(!multi) return;
	if(item[i].canselect)
		item[i].sel = sel;
	UpdateSelect();
...
bool MultiList::IsSelected(int i) const {
	return item[i].sel;
}

bool MultiList::IsSel(int i) const
{
	return IsMultiSelect() ? IsSelected(i) : GetCursor() == i;
}

void MultiList::UpdateSelect() {
	selcount = 0;
	for(int i = 0; i < item.GetCount(); i++)
		if(item[i].sel)
			selcount++;
...
void MultiList::GetItemStyle(int i, Color& fg, Color& bg, dword& st)
{
	st = 0;
...
dword MultiList::PaintItem(Draw& w, int i, const Rect& r)
{
	Color ink, paper;
	dword style;
	GetItemStyle(i, ink, paper, style);
...
void MultiList::PaintRows(Draw &w, Size &sz)
{
	int pos = sb % cy;
	int y = -pos;
	int i = GetSbPos(sz);
...
int  MultiList::GetPageItems() const 
{
	return ncl * GetColumnItems();
}

void MultiList::SetSb() 
{
	switch (mode) {
	case MODE_LIST:
...
void MultiList::RefreshItem(int i, int ex)
{
	if(i >= 0) {
...
void MultiList::SetCursor0(int c, bool sel)
{
	int c0 = cursor;
...
int MultiList::GetSbPos(const Size &sz) const
{
	switch (mode) {
	case MODE_ROWS:
		return (sb / cy)*ncl;
...
void MultiList::LostFocus()
{
	Refresh();
	SyncInfo();
}

int MultiList::RoundedCy()
{
	if (mode != MODE_LIST)
...
void MultiList::FrameLayout(Rect& r)
{
	r.bottom = r.top + RoundedCy();
	frame->FrameLayout(r);
}

void MultiList::FrameAddSize(Size& sz)
{
	frame->FrameAddSize(sz);
}

void MultiList::FramePaint(Draw& draw, const Rect& r)
{
	int ry = RoundedCy();
	Rect rr = r;
	rr.bottom = ry;
...
MultiList& MultiList::RoundSize(bool b)
{
	if(b)
...
void MultiList::SetFrame(CtrlFrame& _frame)
{
	frame = &_frame;
	RefreshLayout();
	Refresh();
...
void MultiList::Sort(const ValueOrder& order)
{
	ItemOrder itemorder;
...
void MultiList::SetData(const Value& key)
{
	int ii = Find(key);
	if (ii >= 0) {
		SelectOne(ii, true);
...
void MultiList::Insert(int ii, const Value& val, bool canselect)
{
	Insert(ii, val, val, canselect);
}

void MultiList::Insert(int ii, const Value& val, const Display& display, bool canselect)
{
	Insert(ii, val, val, display, canselect);
}

void MultiList::Insert(int ii, const Value& key, const Value& val, bool canselect)
{
	int c = -1;
	if(cursor >= ii) {
		c = cursor + 1;
...
void MultiList::Insert(int ii, const Value& key, const Value& val, const Display& display, bool canselect)
{
...
void MultiList::Set(int ii, const Value& val, bool canselect)
{
	Set(ii, val, val, canselect);
}

void MultiList::Set(int ii, const Value& val, const Display& display, bool canselect)
{
	Set(ii, val, val, display, canselect);
}

void MultiList::Set(int ii, const Value& key, const Value& val, bool canselect)
{
	Item& m = item[ii];
	m.value = val;
	m.key = key;
...
void MultiList::Set(int ii, const Value& key, const Value& val, const Display& display, bool canselect)
{
...
void MultiList::Set(const Value &key, const Value& val, const Display& display, bool canselect)
{
	int ii = Find(key);
	if (ii >= 0)
...
void MultiList::Set(const Value &key, const Value& val, bool canselect)
{
	int ii = Find(key);
	if (ii >= 0)
		Set(ii, key, val, canselect);			
...
void MultiList::Add(const Value& val, bool canselect)
{
	Insert(item.GetCount(), val, val, canselect);
}

void MultiList::Add(const Value& val, const Display& display, bool canselect)
{
	Add(val, val, display, canselect);
}

void MultiList::Add(const Value& key, const Value& val, bool canselect)
{
	Insert(item.GetCount(), key, val, canselect);
}

void MultiList::Add(const Value& key, const Value& val, const Display& display, bool canselect)
{
	Add(key, val, canselect);
	item.Top().display = &display;
}

void MultiList::DnD(int _drop, bool _insert)
{
...
bool MultiList::DnDInsert(int i, int py, PasteClip& d, int q)
{
	Rect r = GetItemRect(i);
	int cy = r.GetHeight();
	if(py < r.top + cy / q) {
...
void MultiList::DragAndDrop(Point p, PasteClip& d)
{
	int i = GetItem(p);
	if(i >= 0) {
...
void MultiList::DragRepeat(Point p)
{
	sb = sb + ((mode == MODE_COLUMNS) ? GetDragScroll(this, p, 1).x : GetDragScroll(this, p, 1).y);
}

void MultiList::DragEnter()
{
	RefreshSel();
}

void MultiList::DragLeave()
{
	DnD(-1, false);
	RefreshSel();
}

void MultiList::RemoveSelection()
{
...
int MultiList::Find(const Value &key) const
{
	for (int i = 0; i < item.GetCount(); i++)
		if (item[i].key == key)
			return i;
...
void MultiList::InsertDrop(int ii, const Vector<Value>& keys, const Vector<Value>& data, PasteClip& d, bool self)
{
	if(data.GetCount() == 0)
		return;
...
void MultiList::InsertDrop(int ii, const Vector<Value>& data, PasteClip& d, bool self)
{
	InsertDrop(ii, data, data, d, self);
}

void MultiList::InsertDrop(int ii, const ColumnList& src, PasteClip& d)
{
	Vector<Value> data;
	for(int i = 0; i < src.GetCount(); i++)
		if(src.IsSel(i))
			data.Add(src[i]);
	InsertDrop(ii, data, d, false);
}

void MultiList::InsertDrop(int ii, const MultiList& src, PasteClip& d)
{
	Vector<Value> keys;
	Vector<Value> data;
	for(int i = 0; i < src.GetCount(); i++)
...
void MultiList::InsertDrop(int ii, PasteClip& d)
{
	InsertDrop(ii, GetInternal<MultiList>(d), d);
}
