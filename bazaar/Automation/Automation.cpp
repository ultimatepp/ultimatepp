...
#error Sorry: This platform is not supported!. Look for Automation in Bazaar Upp Forum to search for info and new news
#endif
...
bool Ole::Invoke(int autoType, VARIANT *pvResult, IDispatch *pDisp, String name, int cArgs ...)
{
    ...
    if(FAILED(hr)) {
        ...
        sprintf(buf, "Command \"%s\" not found for object or problem when running it", szName);
        MessageBox(NULL, buf, "Ole error", 0x10010);
        return false;
    }
...
ObjectOle Ole::CreateObject(String application)
{
	...
	if(FAILED(hr)) {
		::MessageBox(NULL, "Application not registered properly", "Error", 0x10010);
		return NULL;
	}
	return app;
}
...
MSSheet::MSSheet()
{
	...
	if (!(Books = Ole::GetObject(App, "Workbooks"))) {
		::MessageBox(NULL, "Workbooks not loaded properly", "Error", 0x10010);
		return;
	}
}
MSSheet::~MSSheet()
...
// Some tricks as Ole Replace does not work properly texts to replace longer than about 250 chars and for some special chars
...
bool OfficeSheet::Init(char *strtype)
{
	if (data) {
		if (type == SheetOPEN)
			delete (OPENSheet*)data;
...
bool OfficeDoc::IsAvailable(char *strtype)
{
	if (strcmp(strtype, "Open") >= 0)
...
bool OfficeDoc::Init(char *strtype)
{
	if (data) {
		if (type == DocOPEN)
			delete (OPENDoc*)data;
