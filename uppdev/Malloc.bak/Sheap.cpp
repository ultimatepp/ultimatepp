...
void *Heap::CheckFree(void *p, int k)
{
...
void Heap::FillFree(void *ptr, int k)
{
#ifdef HEAPDBG
#ifdef CPU_64
...
inline void *Heap::CheckFree(void *p, int k)
{ return p; }

inline void Heap::FillFree(void *ptr, int k)
{}

#endif


Heap::Page *Heap::global_empty[16];
StaticMutex   Heap::global_lock;
...
//		int k = ((int)sz + (sz == 0) - 1) >> 4;
		...
		int k = ((int)sz - 1) >> 4;
		if(level[k]) {
			CTIMING("Alloc from cache");
			return CheckFree(cache[k][--level[k]], k);
		}
		CTIMING("Alloc direct (cache empty)");
		sHeapStat(k);
//		DUMP(Ksz(k));
...
void Heap::Free(void *ptr)
{
	...
	if(((dword)(uintptr_t)ptr) & 8)
		...
		if(page->heap != this)
			Panic("remote free not yet implemented!");
		int k = page->klass;
		ASSERT((4096 - ((uintptr_t)ptr & (uintptr_t)4095)) % Ksz(k) == 0);
		FillFree(ptr, k);
		if(level[k] < CACHE) {
			CTIMING("Free to cache");
			cache[k][level[k]++] = ptr;
			return;
		}
		CTIMING("Free direct (cache full)");
		if(page->freelist)
