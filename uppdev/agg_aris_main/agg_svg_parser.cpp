...
//#include "expat.h" //aris: let's use upp...
//#include <Core/Core.h>
...
//aris: from upp
void parser::parse(XmlParser& p, String& result)
{
	int count=0;
			int count_polyline=0; int count_path=0;

	if(p.IsTag()) {
		start_element(p);
//		String tag = p.ReadTag();  //tag is element?
	
//		if (tag=="g"){
//			for(int i = 0; i < p.GetAttrCount(); i++){
//				String attr = p.GetAttr(i);
//					if(attr =="fill")
//				result <<( tag + ' ' + p.GetAttr(i) + "=\"" + p[i] + "\"" );
...
//			}
	...
	//aris: from upp
	String parser::load_parse(const char *filename, String& result)
	{
	String err1="";
	String extra="";
	//	Title(filename);
	...
	if(l < 0 || l > 16000000){
		err1 = ": Can't open or it's too big";
		return err1;
	}
...
//==========================================================================
	//------------------------------------------------------------------------
	void parser::start_element(XmlParser& p)
	{
		String el = p.ReadTag();
//		parser& self = *(parser*)data;

		if(el=="title")   m_title_flag = true;                  else
		if(el=="g")      { m_path.push_attr(); parse_attr(p); } else
		if(el=="path")
		{
			if(m_path_flag)
			{
		...
		if(el=="rect")      parse_rect(p);        else
		if(el=="line") 		parse_line(p);        else
		if(el=="polyline")  parse_poly(p, false); else
	...
	//------------------------------------------------------------------------
	void parser::end_element(const char *el, XmlParser& p)
	{
//		String el = p.ReadTag();
//		parser& self = *(parser*)data;

		if(el=="title")  m_title_flag = false; else
		if(el=="g")		 m_path.pop_attr();    else
	...
	//------------------------------------------------------------------------
	void parser::parse_attr(XmlParser& p)
	{
		int i;
		for(i = 0; p.GetAttr[i]; i += 2)
		{
			if(p.GetAttr[i]== "style")
			{
				parse_style(p);
			}
			else
	...
	//-------------------------------------------------------------
	void parser::parse_path(XmlParser& p)
	{
		int i;
		...
		for(i = 0; p.GetAttr[i]; i += 2)
		{
			...
			{
				m_tokenizer.set_path_str(p.GetAttr[i + 1]);
	...
	//-------------------------------------------------------------
	void parser::parse_rect(XmlParser& p)
	{
		int i;
		double x = 0.0;
		double y = 0.0;
		...
		for(i = 0; p.GetAttr[i]; i += 2)
		{
			if(!parse_attr(p.GetAttr[i], p.GetAttr[i + 1]))
			{
				if(p.GetAttr[i]=="x")      x = parse_double(p.GetAttr[i + 1]);
				if(p.GetAttr[i]=="y")      y = parse_double(p.GetAttr[i + 1]);
				if(p.GetAttr[i]=="width")  w = parse_double(p.GetAttr[i + 1]);
				if(p.GetAttr[i]=="height") h = parse_double(p.GetAttr[i + 1]);
