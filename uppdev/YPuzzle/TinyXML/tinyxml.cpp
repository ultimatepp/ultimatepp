...
TiXmlNode *TiXmlNode::NodeFromPath(const char *sPath, char chSeparator) const
{
	if(!firstChild || !sPath)
		return NULL;
...
#else
	const char *l_psEndPos = strchr(l_psStartPos, chSeparator);
#endif
...
#else
		strncpy(l_psSubString, l_psStartPos, l_iLength);
#endif
		l_psSubString[l_iLength] = 0;

		l_pNode = (l_pNode) ? l_pNode->FirstChild(l_psSubString) : FirstChild(l_psSubString);
...
#else
		l_psEndPos = strchr(l_psStartPos, chSeparator);
#endif
	}

	return const_cast<TiXmlNode*>((l_pNode) ? l_pNode->FirstChild(l_psStartPos) : FirstChild(l_psStartPos));
}
