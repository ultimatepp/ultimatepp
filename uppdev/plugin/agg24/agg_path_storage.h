...
namespace agg
{
	...
	//------needs more testing and changes?-----aris added-----vertex_upp_storage
	template<class Container> class vertex_upp_storage
	{
	public:
		typedef typename Container::value_type vertex_type;
		typedef typename vertex_type::value_type value_type;
		...
		unsigned last_command() const
		{
			...
			return m_vertices.GetCount() ? 
				m_vertices[m_vertices.GetCount() - 1].cmd : 
				path_cmd_stop;
		}
		...
		unsigned last_vertex(double* x, double* y) const
		{
			...
			return vertex(m_vertices.GetCount() - 1, x, y);
		}

		unsigned prev_vertex(double* x, double* y) const
		{
			if(m_vertices.GetCount() < 2)
			{
				*x = *y = 0.0;
				return path_cmd_stop;
			}
			return vertex(m_vertices.GetCount() - 2, x, y);
		}

		double last_x() const
		{
			return m_vertices.GetCount() ? m_vertices[m_vertices.size() - 1].x : 0.0;
		}

		double last_y() const
		{
			return m_vertices.GetCount() ? m_vertices[m_vertices.size() - 1].y : 0.0;
		}

		unsigned total_vertices() const
		{
		//	return m_vertices.size(); //aris it didn't work with size and upp
			return m_vertices.GetCount();
		}

		unsigned vertex(unsigned idx, double* x, double* y) const
		{
			const vertex_type& v = m_vertices[(int)idx];  //aris added (int)		
			*x = v.x;
			*y = v.y;
			return v.cmd;
		}
