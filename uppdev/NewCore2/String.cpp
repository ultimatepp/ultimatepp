...
unsigned String::LHashValue() const
{
	int l = LLen();
	if(l < 15) {
		dword w[4];
...
char *String::Alloc(int count, char& kind)
{
	if(count < 32) {
		kind = MEDIUM;
...
char *String::Insert(int pos, int count, const char *s)
{
...
void String::Insert(int pos, const char *str, int len)
{
	Insert(pos, len, str);
}
...
void String::SetSLen(int l)
{
	SLen() = l;
	memset(chr + l, 0, 15 - l);
}

void String::Remove(int pos, int count)
{
...
void String::Set(int pos, int chr)
{
