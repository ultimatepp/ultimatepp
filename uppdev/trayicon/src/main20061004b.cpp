...
//#define IMAGEFILE  <TrayWindow/tray.iml>
//#include <Draw/iml.h>
using namespace std;
//class TrayWindow : private Ctrl {
...
TrayWindow::TrayWindow():TopWindow()
{
...
bool TrayWindow::FileExists(char* filename)
{
...
void TrayWindow::Icon(char* trayName, char* trayImageFile)
{
//    trayName= "Testing TrayWindow";        //only work when they are put here but why?????
//    trayImageFile= "some_image.png";
	if (! FileExists(trayImageFile) ) {
		printf(" You must put \"%s\" under the same directory as TrayWindow!\n",trayImageFile);
		return;
	}
...
void TrayWindow::TrayHandleEvent(XEvent *xevent)
{
...
class App : public TrayWindow{
	MenuBar menu;
	StatusBar status;
public:
	void Info() {
//		Icon(Tray::Icon1());
		PromptOK("TrayWindow launched this prompt!");
//		Icon(Tray::Icon());
	}
	...
	App() : TrayWindow(){
		...
		trayName= "Testing TrayWindow";        //can't be here but why?????
		trayImageFile= "some_image.png";
