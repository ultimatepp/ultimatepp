...
void GridHeader::GSort(int scol)
{
	int col;
	...
	if(cnt > 1 && scol != 0)
	{
		...
		for(int i = fixedRows + 1; i < totalRows; i++)
		{
			int n = vitems[i].id;
			int m = vitems[is].id;
		...
		GSort(col, order, fixedRows);

}

void GridHeader::GSort(int col, int order, int from, int count)
{
	if(count == 0)
		return;
...
int GridHeader::InMultisort(int col)
{
	for(int i = 0; i < sortOrder.GetCount(); i++)
		if(col == sortOrder[i])
			return i;
...
bool GridHeader::ClearMultisort()
{
